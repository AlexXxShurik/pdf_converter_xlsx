...
from converter.table_geometry import (
    ...
    attribute_columns, parse_legend,
)
...
hy = headers[0]
band_end = headers[1] - 20 if len(headers) > 1 else PAGE_H
...
alloys, groups = parse_legend(leg_blocks, [])
...
print('header band:')
for b in sorted(hb, key=lambda b: b['cx']): print('  ', repr(b['text']), round(b['cx'],1))
...
print('numbers:')
for b in sorted(number_blocks, key=lambda b: (b['cy'], b['cx'])):
    print('  ', b['text'], round(b['cx'],1), round(b['cy'],1))
